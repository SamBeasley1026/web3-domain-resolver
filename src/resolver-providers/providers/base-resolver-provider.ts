...
} from "../../shared/enumerations/enumerations";
export abstract class BaseResolverProvider implements IResolverProvider {
  ...
  protected getReadContractConnection(
  ...
  protected getWriteContractConnection(
    networkName: NetworkName | string
  ): ContractConnection | undefined {
    return this.writeContractConnections.find((x) => x.network == networkName);
  }

  protected getWriteContractWithSigner(
    networkName: NetworkName | string,
    signer: Wallet | ethers.Signer
  ) {
    const writeContractConnection =
      this.getWriteContractConnection(networkName);
    ...
    if (
      ...
      if (!(signer as ethers.Signer)?.provider) {
        signerToUse = (signer as ethers.Signer)?.connect(
          writeContractConnection.provider
        );
      }
    }
  ...
  public async resolve(
    domainOrTld: string
  ): Promise<IResolvedResource | undefined> {
    console.log("BaseResolverProvider.resolve", domainOrTld);

    try {
      const mappedName = NameTools.mapName(domainOrTld);

      console.log("BaseResolverProvider.resolve mappedName", mappedName);
      ...
      console.log("BaseResolverProvider.resolve result", result);
  ...
  public async resolveFromTokenId(
    tokenId: string,
  ...
  public async reverseResolve(
    ...
    for (const readContractConnection of readContracts) {
      try {
        const reverseOfRes: ethers.BigNumber | string | undefined =
          await readContractConnection.contract.reverseOf(address);
        if (reverseOfRes) {
  ...
  public async getTokenUri(
    ...
    const readContractConnection =
      await this.getReadContractConnectionFromToken(tokenId, network);
  ...
  public async isApprovedOrOwner(
    tokenId: string,
    ...
    try {
      return await readContractConnection.contract.isApprovedOrOwner({
        tokenId,
  ...
  public async getRecord(
    ...
    const readContractConnection: ContractConnection | undefined =
      await this.getReadContractConnectionFromToken(tokenId, network);
    if (!readContractConnection) {
      return undefined;
    }
  ...
  public async transfer(
    ...
    const writeContract = this.getWriteContractWithSigner(
      resource.network,
      signer
    );
    if (!writeContract) {
    ...
    try {
      return await writeContract.transferFrom(
  ...
  protected async generateResolvedResource(
    ...
    const writeContractConnection = await this.getWriteContractConnection(
      readContractConnection.network
    );
    if (!writeContractConnection) {
      return undefined;
    }
    ...
    const resolverResourceType: ResolvedResourceType =
      NameTools.getResolvedResourceType(mappedName.type);
  ...
  protected async getTokenIdNetwork(
    tokenId: string
  ): Promise<NetworkName | string | undefined> {
  ...
  protected async getReadContractConnectionFromToken(
    tokenId: string,
    network?: NetworkName | string | undefined
  ): Promise<ContractConnection | undefined> {
  ...
  abstract generateTokenId(
    mappedName: MappedName,
    network?: NetworkName
  ): Promise<string | undefined>;

  abstract getNetworkFromName(
    mappedName: MappedName
  ): Promise<NetworkName | string | undefined>;
